...
-- Dumping structure for table network111.friends
CREATE TABLE IF NOT EXISTS `friends` (
  `id` int(11) NOT NULL,
  `user_login` int(11) NOT NULL,
  `friend_login` int(11) NOT NULL,
  PRIMARY KEY (`id`),
  KEY `FK_friends_user` (`user_login`),
  KEY `FK_friends_user_2` (`friend_login`),
  CONSTRAINT `FK_friends_user` FOREIGN KEY (`user_login`) REFERENCES `user` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE,
  CONSTRAINT `FK_friends_user_2` FOREIGN KEY (`friend_login`) REFERENCES `user` (`id`) ON DELETE NO ACTION ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;

-- Dumping data for table network111.friends: ~2 rows (approximately)
